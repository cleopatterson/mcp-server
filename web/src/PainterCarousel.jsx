...
function PainterCarousel() {
  ...
  if (!toolOutput || toolOutput.type !== "painter_list") {
    return <div className="p-6 text-gray-500">Loading painters...</div>;
  }
  ...
  return (
    <div
      className={`${displayMode === "fullscreen" ? "min-h-screen" : ""} bg-gradient-to-br from-blue-50 to-indigo-50 p-4`}
    >
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            Top Painters in {location.suburb || `Postcode ${location.postcode}`}
          </h2>
          <p className="text-gray-600">
            Found {total} highly-rated painter{total !== 1 ? "s" : ""} • Swipe
            to browse
          </p>
        </div>
        ...
        <div className="relative">
          ...
          {painters.length > 1 && (
            <>
              ...
              <button
                onClick={() =>
                  scrollToIndex(Math.min(painters.length - 1, currentIndex + 1))
                }
                ...
                className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-lg flex items-center justify-center transition-all ${
                  currentIndex === painters.length - 1
                    ? "opacity-0 pointer-events-none"
                    : "opacity-100 hover:bg-gray-50"
                }`}
              >
                  ...
                  <div className="px-6 py-3 border-b">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-sm text-gray-700">
                        {painter.location.suburb && (
                          <div className="font-medium">
                            {painter.location.suburb}
                          </div>
                        )}
                        <div className="text-gray-500">
                          {painter.location.area}, {painter.location.region}
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            width: `${painter.score * 100}px`,
                            maxWidth: "100px",
                          }}
                        ></div>
                        <span className="font-bold text-green-700">
                          {(painter.score * 100).toFixed(0)}%
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="px-6 pb-4">
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 text-xs">
                        <span className="text-yellow-800">
                          ⚡ Quick to respond - {painter.engagement_rate}%
                          engagement rate
                        </span>
                      </div>
                    </div>
